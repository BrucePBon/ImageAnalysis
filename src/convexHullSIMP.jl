function convexHullSIMP!( points::Array{Tuple{T,T,T},1}; typ=UInt16 ) where {T<:Real}
    ...
    # By now, the total length of faces is 2*length(points), this should provide
	# more than enough space for the all convex hull faces
	...
	# Making sure the 4 first points points are not the same, colinear or coplanar
	# These will form the initial tetrahedron that nucleates the convex hull
	fixSameColinearCoplanar!( points )
	...
	@inbounds for pidx in 5:length(points)
		...
		for fidx in 1:lastFace;
			...
			# new_point is outside/visible (positive dot product) from the current face
		...
		# After adding the index of visible faces to "visibleFaces", we can proceed to
		# construct new faces over these visible faces to the new_point
		lastFace = addNewFacesSIMP!( points, faces, norms, center, lastFace,
						             pidx, visibleFaces, numVisibleFaces );

	end # @inbounds for pidx in 5:length(points)
...
@inline function addNewFacesSIMP!( points,
	...
	"""
	   1-. Isolating non-repeated edges among visibleFaces. These form the boundary
	       between visible faces and non-visible faces.
	"""
...
function fixSameColinearCoplanar!( points )
